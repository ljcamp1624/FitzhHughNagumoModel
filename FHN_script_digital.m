...
%%
% vid = VideoWriter('DigitalResponse', 'MPEG-4');
% vid.FrameRate = 25;
% open(vid);
numFrames = 200;
...
for i = 0:numFrames
    ...
    c1 = c;
    v1 = 0.2*(i/numFrames);
    x1 = fsolve(@(x) b1*x^3/3 + (1 - b1)*x + (a1 - b1*v1), -2);
    y1 = (x1 + a1)/b1;
...
%         plot(0, 0, '--', 'color', [1 1 1]/2, 'linewidth', 1);
        ...
        subplot(1, 2, 2);
        hold on; 
        plot3(vInt, vInt - vInt.^3/3 + v1, -numFrames + i + zeros(size(vInt)), 'color', [1 1 1]/2, 'linewidth', 0.5);
    ...
    set(f7, 'string', ['V_{ext} = ', num2str(v1, '%.3f')], 'Interpreter', 'tex', 'fontsize', 16);
